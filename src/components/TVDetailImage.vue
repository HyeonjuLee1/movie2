...
const trailerVideoKey = computed(() => {
    return (tvStore.tvTrailerData.value?.results.find(r => r.type === "Trailer"))?.key;
})
</script>

<template>
    <section class="player-contents">
        <div v-if="!tvStore.isTVShowDetailLoading" class="player-bg-image"
            :style="{ backgroundImage: `linear-gradient(to bottom left,rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 1)), url(${GET_TVBACKPOSTER_URI}${props.movieData?.backdrop_path})` }">
            ...
            <div class="movie-detail-area">
                <span class="text" style="margin-bottom: 20px;">{{ props.movieData?.name }}</span>
